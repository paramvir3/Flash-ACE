...
class ACE_Descriptor(nn.Module):
    def __init__(self, r_max, l_max, num_radial, hidden_dim):
        ...
        # Linear Mixing to recover full feature interactions, matching the
        # ACE descriptor block in MACE where the contracted basis and the
        # residual stream are both linearly remixed.
        self.mix = o3.Linear(
            self.tp_b.irreps_out, self.irreps_out, internal_weights=True, shared_weights=False
        )
        self.residual = o3.Linear(
            self.irreps_out, self.irreps_out, internal_weights=False, shared_weights=False
        )
    ...
    def forward(self, node_attrs, edge_index, edge_vec, edge_len):
        ...
        return self.mix(B_basis) + self.residual(A_basis)
